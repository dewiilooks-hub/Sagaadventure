...
const RentalPage: React.FC<RentalPageProps> = ({ onBack }) => {
  ...
  const handleSaveRental = async (silent = false) => {
    ...
    try {
      if (!db) {
        ...
        if (!silent) alert(`Invoice ${invId} Tersimpan (Lokal)`);
      } else {
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'saga_rentals'), payload);
        if (!silent) alert(`Invoice ${invId} Tersimpan di Cloud`);
      }
    } catch (e) {
      console.error(e);
  ...
  const handleDownloadPDF = async () => {
    // Simpan dulu secara diam-diam baru buka print
      ...
      <div className="bg-stone-200 min-h-screen pb-20 font-sans relative">
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t flex gap-2 max-w-md mx-auto print:hidden shadow-lg z-50">
          ...
          <button 
            ...
            className="flex-[2] bg-orange-700 text-white py-3 rounded-xl font-bold uppercase text-[10px] flex items-center justify-center gap-2 shadow-md transition-all active:scale-95"
          >
            <Download size={14}/> {isSaving ? 'Processing...' : 'Download PDF'}
          </button>
