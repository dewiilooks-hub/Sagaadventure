...
const RentalPage: React.FC<RentalPageProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    if (damageInvoiceRef.length >= 8) { // Assuming INV-XXXXXX format
      setIsSearchingInv(true);
      const found = recapData.find(r => r.invoiceId === damageInvoiceRef);
      if (found) {
        // If totalPerDay was stored, use it, otherwise calculate from rentalTotal / duration
  ...
  const handleSaveRental = async (silent = false) => {
    ...
    const payload = {
      ...
      totalPerDay: totalPerDay, // Added for easier retrieval in damage calc
      duration: duration,       // Added for easier retrieval in damage calc
  ...
  if (subView === 'terms') {
    const memoRules = [
      ...
      { 
        ...
        text: "Apabila terjadi kerusakan, pihak SAGA akan melakukan pengecekan. Kerusakan ringan yang masih bisa diperbaiki akan dikenakan biaya servis. Namun jika kerusakan tidak bisa diperbaiki, penyewa wajib mengganti unit baru atau biaya penuh sesuai harga pasar." 
      },
      { 
        title: "Kebersihan & Toleransi", 
        text: "Kondisi kotor biasa (debu/tanah wajar) atau terkena air hujan masih diberikan toleransi tanpa biaya tambahan. Namun kondisi kotor ekstrem (lumpur pekat/noda permanen) akan dikenakan biaya cleaning service sesuai beban kerja alat." 
      }
    ];
    ...
    return (
      <div className="bg-stone-100 min-h-screen pb-24 relative font-sans animate-in slide-in-from-right">
        ...
        <div className="m-4 bg-white shadow-2xl rounded-sm border-t-[12px] border-stone-900 p-8 space-y-10 relative overflow-hidden max-w-2xl mx-auto">
           <div className="flex justify-between items-start border-b-2 border-stone-100 pb-8">
              ...
              <div className="text-right">
                 ...
                 <p className="text-[8px] font-black text-stone-300 mt-2 uppercase tracking-[0.4em]">Internal & Client Reference</p>
              </div>
           </div>

           <div className="bg-stone-50 p-6 rounded-2xl border border-stone-100 space-y-4 print:bg-transparent print:border-none print:p-0">
              <div className="grid grid-cols-2 gap-4">
                 <div className="space-y-1">
                    <label className="text-[8px] font-black text-stone-300 uppercase tracking-widest">Penyewa (Nama)</label>
                    <div className="relative">
                       <User size={12} className="absolute left-0 top-1/2 -translate-y-1/2 text-stone-300" />
                       <input 
                         type="text" 
                         placeholder="MASUKKAN NAMA" 
                         className="w-full bg-transparent border-b border-stone-200 focus:border-stone-900 outline-none font-bold text-stone-900 uppercase py-1 pl-4 text-xs"
                         value={customerName}
                 ...
                 <div className="space-y-1">
                    <label className="text-[8px] font-black text-stone-300 uppercase tracking-widest">No. Invoice</label>
                    <div className="relative">
                       <FileText size={12} className="absolute left-0 top-1/2 -translate-y-1/2 text-stone-300" />
                       <input 
                         type="text" 
                         placeholder="INV-XXXXXX" 
                         className="w-full bg-transparent border-b border-stone-200 focus:border-stone-900 outline-none font-mono font-bold text-stone-700 py-1 pl-4 text-xs"
              ...
              <div className="grid grid-cols-2 gap-4 text-[9px] font-bold text-stone-400">
                 <p>Dibuat Oleh: <span className="text-stone-900 font-black">SAGA ADVENTURE</span></p>
                 <p>Tanggal: <span className="text-stone-900 font-black">{new Date().toLocaleDateString('id-ID')}</span></p>
              </div>
           </div>
           ...
           <div className="bg-stone-900 p-6 rounded-2xl text-white/90 text-[10px] leading-relaxed font-bold italic">
           ...
           <div className="grid grid-cols-2 gap-10 pt-16 text-center text-[10px] font-black text-stone-300 uppercase tracking-widest border-t border-dashed border-stone-100">
              <div className="space-y-20">
                 <p>DIBUAT OLEH</p>
                 <div className="flex flex-col items-center">
                    <div className="border-b border-stone-200 w-32 mb-2"></div>
                    <p className="text-stone-900 italic font-bold">SAGA Crew</p>
                 </div>
              </div>
              <div className="space-y-20">
                 <p>DISETUJUI OLEH</p>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t flex gap-2 max-w-md mx-auto print:hidden shadow-lg z-[100]">
           ...
           <button 
             ...
             className="flex-[2] bg-emerald-900 text-white py-4 rounded-xl font-black uppercase text-[10px] flex items-center justify-center gap-2 shadow-md hover:bg-emerald-800 transition-colors"
           >
             ...
             {damageItems.map((item, idx) => (
               <div key={idx} className="flex justify-between items-center bg-stone-50 p-4 rounded-xl border border-stone-100">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-black">Rp{item.cost.toLocaleString('id-ID')}</span>
  ...
  // --- DAMAGE INVOICE (PRINTABLE) ---
  if (subView === 'damage_invoice') {
    return (
      <div className="bg-stone-100 min-h-screen pb-20">
        <div className="sticky top-0 bg-white border-b p-4 flex items-center justify-between shadow-sm z-50 print:hidden">
          <button onClick={() => setSubView('damage')} className="p-2 hover:bg-gray-100 rounded-full text-orange-800"><ChevronLeft/></button>
          <span className="text-[10px] font-black uppercase tracking-widest opacity-40">Penalty Report</span>
          <div className="w-8"></div>
        </div>

        <div className="m-4 bg-white shadow-xl text-sm rounded-sm border-t-8 border-orange-800 p-8 space-y-8 relative overflow-hidden">
          ...
          <div className="space-y-6">
             <div className="bg-stone-50 p-6 rounded-2xl border border-stone-200">
                <h4 className="text-[10px] font-black text-stone-400 uppercase tracking-widest mb-4">Rincian Keterlambatan & Kerusakan</h4>
                <div className="space-y-3">
                   {lateHours > 0 && (
                     <div className="flex justify-between items-center text-xs font-bold border-b border-stone-200 pb-2">
                        <span className="uppercase text-stone-500">Denda Telat ({lateHours} Jam) {lateHours > 4 ? "" : "(Dispensasi)"}</span>
             ...
             <div className="bg-orange-800 p-8 rounded-[2rem] text-white flex justify-between items-center shadow-xl">
                <div>
                   <span className="text-[9px] font-black uppercase tracking-widest opacity-60">Total Yang Harus Dibayar</span>
                   <p className="text-2xl font-black mt-1">Rp{grandTotalDamage.toLocaleString('id-ID')}</p>
                </div>
                <AlertTriangle size={32} className="opacity-20" />
             </div>
          </div>

          <div className="bg-orange-50 p-6 rounded-2xl border border-orange-100 italic text-[10px] text-orange-900 leading-relaxed font-bold">
            "Sesuai dengan Syarat & Ketentuan SAGA Adventure, keterlambatan pengembalian alat dan/atau kerusakan gear yang disewa wajib diselesaikan saat pengembalian barang."
          </div>
          ...
          <div className="grid grid-cols-2 gap-8 mt-12 text-center text-[10px] text-stone-400 font-bold uppercase tracking-widest">
               ...
               <div>
                 <p className="mb-16">SAGA Adventure</p>
                 <div className="border-b border-stone-300 mx-8"></div>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t flex gap-2 max-w-md mx-auto print:hidden shadow-lg">
           <button onClick={async () => { await handleSaveDamage(true); window.print(); }} className="flex-1 bg-orange-800 text-white py-4 rounded-xl font-black uppercase text-[10px] flex items-center justify-center gap-2">
  ...
  if (subView === 'invoice') {
    return (
      <div className="bg-stone-200 min-h-screen pb-20 relative font-sans">
        ...
        <div className="m-4 bg-[#fdfbf7] shadow-xl text-sm relative overflow-hidden rounded-sm border border-stone-300 p-8">
           ...
           <div className="border-b-2 border-stone-300 pb-6 mb-6 flex justify-between items-start mt-4">
            ...
            <div className="mb-8 space-y-4">
               <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                     <label className="text-[8px] font-black text-stone-400 uppercase tracking-widest">Penyewa</label>
                  ...
                  <div className="space-y-1">
                     <label className="text-[8px] font-black text-stone-400 uppercase tracking-widest">No HP</label>
               ...
               <div className="grid grid-cols-3 gap-3">
                  <div className="space-y-1">
                     <label className="text-[8px] font-black text-stone-400 uppercase tracking-widest">Ambil</label>
                  ...
                  <div className="space-y-1">
                     <label className="text-[8px] font-black text-stone-400 uppercase tracking-widest">Kembali</label>
                  ...
                  <div className="space-y-1">
                     <label className="text-[8px] font-black text-stone-400 uppercase tracking-widest">Durasi</label>
                     <div className="bg-emerald-50 text-emerald-900 font-black text-center py-1 rounded border border-emerald-100">{duration} <span className="text-[7px]">HARI</span></div>
                  </div>
               </div>
            </div>
                ...
                <tbody className="text-stone-700 font-bold">
                  ...
                  {customItems.map((item, idx) => (
                    <tr key={idx} className="border-b border-stone-50 bg-stone-50/50">
                      <td className="py-3 uppercase italic text-stone-500 font-medium">{item.name}</td>
            ...
            <div className="space-y-3 bg-stone-50/50 p-6 rounded-2xl print:hidden">
                <h3 className="text-[8px] font-black text-stone-400 uppercase tracking-widest flex items-center gap-2"><PackagePlus size={10}/> Add-ons & Custom Discount</h3>
                <div className="grid grid-cols-2 gap-2">
                   <input type="text" placeholder="Add-on Name" className="bg-white rounded-lg px-3 py-2 text-[10px] outline-none border" value={newCustomName} onChange={e => setNewCustomName(e.target.value)} />
                   <input type="number" placeholder="Price" className="bg-white rounded-lg px-3 py-2 text-[10px] outline-none border" value={newCustomPrice} onChange={e => setNewCustomPrice(e.target.value)} />
                </div>
                <div className="flex gap-2">
                   <button onClick={addCustomItem} className="flex-1 bg-emerald-700 text-white rounded-lg py-2 text-[10px] font-bold">+ Add Item</button>
                   <input type="number" placeholder="Manual Discount (Rp)" className="flex-1 bg-white rounded-lg px-3 py-2 text-[10px] outline-none border" value={discount || ''} onChange={e => setDiscount(parseInt(e.target.value) || 0)} />
                </div>
            </div>

            <div className="mt-8 bg-stone-900 p-8 rounded-[2rem] text-white space-y-3 shadow-2xl relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-5 rotate-12"><Receipt size={80}/></div>
                ...
                {effectiveDiscount > 0 && (
                   <div className="flex justify-between text-[9px] font-bold uppercase tracking-widest text-emerald-400">
                      <span>Diskon {discount > 0 ? '(Manual)' : '(Auto 3+ Hari 10%)'}</span>
            ...
            <div className="grid grid-cols-2 gap-8 mt-16 text-center text-[10px] text-stone-400 font-bold uppercase tracking-widest">
               <div>
                 <p className="mb-16">Penyewa</p>
                 <div className="border-b border-stone-300 mx-8"></div>
                 <p className="mt-2 text-stone-900 italic">{customerName || '...........'}</p>
               </div>
               <div>
                 <p className="mb-16">SAGA Adventure</p>
                 <div className="border-b border-stone-300 mx-8"></div>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t flex gap-2 max-w-md mx-auto print:hidden shadow-lg">
           <button onClick={() => setSubView('catalog')} className="flex-1 bg-stone-100 text-stone-700 py-4 rounded-xl font-bold uppercase text-[10px]">Back</button>
    ...
    return (
      <div className="p-8 space-y-6 bg-white min-h-screen animate-in fade-in">
        ...
        <div className="space-y-4 pb-20">
           <h3 className="text-[9px] font-black text-stone-300 uppercase tracking-widest">Transaction History</h3>
           {filteredRecap.length === 0 ? (
             <div className="text-center py-20 border-2 border-dashed rounded-[2rem] opacity-20"><ShoppingCart size={48} className="mx-auto"/><p className="text-xs font-bold mt-2 uppercase tracking-widest">Empty</p></div>
           ) : (
             filteredRecap.map((item, idx) => (
               <div key={item.id || idx} className="bg-white border border-stone-100 p-6 rounded-[2rem] flex justify-between items-center group shadow-sm">
                  <div>
                     ...
                     <p className="text-[9px] font-bold text-stone-400 uppercase">{item.customer || 'No Name'}</p>
                  </div>
