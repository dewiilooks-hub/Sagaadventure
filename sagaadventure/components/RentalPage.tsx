    ...
    return (
      <div className="p-8 space-y-6 bg-stone-50 min-h-screen animate-in fade-in">
        <div className="flex items-center justify-between mb-8">
           <button onClick={() => setSubView('dashboard')} className="p-3 bg-white rounded-full shadow-sm text-stone-600"><ChevronLeft size={18}/></button>
  ...
  if (subView === 'damage_invoice') {
    return (
      <div className="bg-stone-100 min-h-screen pb-20 font-sans">
        ...
        {/* PAGE 2: S&K */}
        <div className="m-4">
           {renderSKPage(`DMG-${damageInvoiceRef}`, customerName)}
        </div>
           ...
           <div className="border-b-2 border-stone-300 pb-6 mb-6 flex justify-between items-start mt-4">
                  ...
                  {customItems.map((item, idx) => (
                    <tr key={idx} className="border-b border-stone-50 bg-stone-50/50">
                      <td className="py-3 flex items-center gap-2 uppercase italic text-stone-500 font-medium">
                         <button onClick={() => removeCustomItem(idx)} className="text-stone-300 hover:text-red-500 print:hidden"><Trash2 size={10}/></button>
                         {item.name}
                      </td>
            ...
            <div className="space-y-3 bg-stone-50 p-6 rounded-2xl border border-dashed border-stone-200 print:hidden mb-8">
                ...
                <div className="flex gap-2">
                   <button onClick={addCustomItem} className="flex-1 bg-emerald-700 text-white rounded-lg py-3 text-[9px] font-black uppercase tracking-widest shadow-md">+ Tambah Item</button>
  ...
  if (subView === 'recap') {
    ...
    return (
      <div className="p-8 space-y-6 bg-white min-h-screen animate-in fade-in">
        <div className="flex items-center justify-between mb-8">
           <button onClick={() => setSubView('dashboard')} className="p-3 bg-stone-50 rounded-full text-stone-600"><ChevronLeft size={18}/></button>
        ...
        <div className="space-y-4 pb-20">
           ...
           {filteredRecap.length === 0 ? (
             ...
             filteredRecap.map((item, idx) => (
               <div key={item.id || idx} className="bg-white border border-stone-100 p-6 rounded-[2rem] flex justify-between items-center group shadow-sm hover:shadow-md transition-shadow">
                  <div>
