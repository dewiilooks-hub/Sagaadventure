...
const RentalPage: React.FC<RentalPageProps> = ({ onBack }) => {
  ...
  const [selectedRecapItem, setSelectedRecapItem] = useState<any | null>(null);
    ...
    return (
      <div className="p-8 space-y-6 bg-stone-50 min-h-screen animate-in fade-in relative overflow-x-hidden">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] shadow-xl space-y-6 border border-stone-100">
           ...
           <button onClick={() => setSubView('monthly_report')} className="w-full bg-stone-900 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-[9px] flex items-center justify-center gap-2 hover:bg-black transition-all">
        ...
        <div className="space-y-4 pb-20">
           ...
           {filteredRecap.length === 0 ? (
             ...
             filteredRecap.map((item, idx) => (
               <div key={item.id || idx} className="bg-white border border-stone-100 p-6 rounded-[2rem] flex flex-col gap-4 group shadow-sm hover:shadow-md transition-all">
                  ...
                  <div className="flex gap-2 pt-2 border-t border-stone-50">
                     <button onClick={() => setSelectedRecapItem(selectedRecapItem === item ? null : item)} className="flex-1 bg-stone-50 text-stone-600 py-3 rounded-xl text-[8px] font-black uppercase tracking-widest flex items-center justify-center gap-1.5 active:scale-95 transition-all">
                       <Eye size={12}/> {selectedRecapItem === item ? 'Tutup Rincian' : 'Rincian'}
                     </button>
                     <button onClick={() => handleReprint(item)} className="flex-1 bg-stone-100 text-stone-900 py-3 rounded-xl text-[8px] font-black uppercase tracking-widest flex items-center justify-center gap-1.5 active:scale-95 transition-all">
                       <Printer size={12}/> Cetak Ulang
                     </button>
                     {isAdmin && (
                        <button onClick={() => handleDeleteTransaction(item.id || item.invoiceId)} className="p-3 bg-red-50 text-red-400 rounded-xl hover:bg-red-500 hover:text-white transition-all"><Trash2 size={12}/></button>
                     )}
                  </div>

                  {selectedRecapItem === item && (
                    <div className="p-4 bg-stone-50 rounded-2xl space-y-4 animate-in slide-in-from-top-2 duration-300">
                       <div className="space-y-2">
                          <p className="text-[8px] font-black text-stone-300 uppercase tracking-widest">Detail Item</p>
                          {item.type === 'rental' ? (
                            <div className="space-y-1">
                               ...
                               <div className="flex justify-between text-[10px] font-black text-stone-900">
                                  <span>DURASI SEWA</span>
                                  <span>{item.duration || 1} HARI</span>
                                </div>
                            </div>
                          ) : (
                            <div className="space-y-1">
                               ...
                               {item.lateFee > 0 && (
                                 <div className="flex justify-between text-[10px] font-bold uppercase text-orange-700">
                                    <span>Denda Telat ({item.lateHours} Jam)</span>
    ...
    return (
      <div className="bg-white min-h-screen pb-20 font-sans p-8 print:p-0">
        <div className="sticky top-0 bg-white border-b p-4 flex items-center justify-between shadow-sm z-50 print:hidden mb-10">
          <button onClick={() => setSubView('recap')} className="p-2 hover:bg-gray-100 rounded-full text-stone-600"><ChevronLeft/></button>
        ...
        <div className="bg-white border-t-[12px] border-stone-900 p-10 space-y-12 relative overflow-hidden min-h-[900px] flex flex-col print:border-none print:p-0">
           <div className="flex justify-between items-start border-b-2 border-stone-100 pb-8">
              ...
              <div className="text-right">
                 <p className="text-[10px] font-black text-stone-900 uppercase tracking-[0.2em]">{months[selectedMonth]} {selectedYear}</p>
                 <p className="text-[8px] font-bold text-stone-300 mt-1 uppercase tracking-widest">SAGA Adventure Operational</p>
              </div>
           </div>

           <div className="grid grid-cols-3 gap-6">
              <div className="bg-stone-50 p-6 rounded-2xl border border-stone-100 space-y-1">
                 <span className="text-[8px] font-black text-stone-300 uppercase tracking-widest block">Total Rental</span>
                 <p className="text-lg font-black text-stone-900">Rp{incomeRental.toLocaleString('id-ID')}</p>
              </div>
              <div className="bg-stone-50 p-6 rounded-2xl border border-stone-100 space-y-1">
                 <span className="text-[8px] font-black text-stone-300 uppercase tracking-widest block">Denda/Damage</span>
                 <p className="text-lg font-black text-stone-900">Rp{incomeDamage.toLocaleString('id-ID')}</p>
              </div>
              <div className="bg-stone-900 p-6 rounded-2xl text-white space-y-1 shadow-lg">
                 <span className="text-[8px] font-black text-white/40 uppercase tracking-widest block">Total Omzet</span>
                 <p className="text-lg font-black">Rp{(incomeRental + incomeDamage).toLocaleString('id-ID')}</p>
              </div>
           </div>

           <div className="space-y-4">
              <h3 className="text-[10px] font-black text-stone-900 uppercase tracking-[0.3em] bg-stone-100 px-4 py-2 w-fit rounded-full mb-6">Rincian Transaksi Bulanan</h3>
              <table className="w-full text-left">
                 <thead className="border-b border-stone-100">
                    <tr className="text-[8px] font-black text-stone-300 uppercase tracking-widest">
                       <th className="py-3">No. Invoice</th>
                       <th className="py-3">Penyewa</th>
                 ...
                 <tbody className="text-[10px] font-bold text-stone-800 uppercase">
                    {filteredRecap.map((item, idx) => (
                       <tr key={idx} className="border-b border-stone-50">
                          <td className="py-3 font-mono">{item.invoiceId}</td>
           ...
           <div className="mt-auto pt-20 grid grid-cols-2 gap-20 text-center text-[10px] font-black text-stone-300 uppercase tracking-widest">
              <div className="space-y-20">
                 <p>Dibuat Oleh</p>
                 <div className="flex flex-col items-center">
                    <div className="border-b border-stone-200 w-40 mb-2"></div>
                    <p className="text-stone-900 italic">SAGA CREW</p>
                 </div>
              </div>
              <div className="space-y-20">
                 <p>Divalidasi Oleh</p>
                 <div className="flex flex-col items-center">
                    <div className="border-b border-stone-200 w-40 mb-2"></div>
                    <p className="text-stone-900 italic">OWNER / MANAGER</p>
                 </div>
              </div>
           </div>
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t flex gap-2 max-w-md mx-auto print:hidden shadow-lg z-[100]">
           <button onClick={() => setSubView('recap')} className="flex-1 bg-stone-100 text-stone-700 py-4 rounded-xl font-bold uppercase text-[10px]">Kembali</button>
           <button onClick={() => window.print()} className="flex-[2] bg-stone-900 text-white py-4 rounded-xl font-black uppercase text-[10px] flex items-center justify-center gap-2 shadow-md">
             <Printer size={16} /> Print Laporan Closing
           </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="bg-stone-100 min-h-screen pb-24 relative font-sans animate-in slide-in-from-right">
        <div className="sticky top-0 bg-white border-b p-4 flex items-center justify-between shadow-sm z-50 print:hidden">
           <button onClick={() => setSubView('dashboard')} className="p-2 hover:bg-gray-100 rounded-full text-stone-600"><ChevronLeft/></button>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t flex gap-2 max-w-md mx-auto print:hidden shadow-lg z-[100]">
           ...
           <button onClick={async () => { if(customerName && activeInvoiceId) { await handleSaveRental(true); } window.print(); }} className="flex-[2] bg-emerald-900 text-white py-4 rounded-xl font-black uppercase text-[10px] flex items-center justify-center gap-2 shadow-md hover:bg-emerald-800 transition-colors">
             ...
             {damageItems.map((item, idx) => (
               <div key={idx} className="flex justify-between items-center bg-stone-50 p-4 rounded-xl border border-stone-100">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => setDamageItems(damageItems.filter((_, i) => i !== idx))} className="text-stone-300 hover:text-red-500 transition-all"><Trash2 size={12}/></button>
                  </div>
               </div>
             ))}
          </div>
  ...
  if (subView === 'damage_invoice') {
    return (
      <div className="bg-stone-100 min-h-screen pb-20 font-sans">
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t flex gap-2 max-w-md mx-auto print:hidden shadow-lg z-[100]">
           <button onClick={() => setSubView('damage')} className="flex-1 bg-stone-100 text-stone-700 py-4 rounded-xl font-bold uppercase text-[10px]">Kembali</button>
           <button onClick={async () => { await handleSaveDamage(true); window.print(); }} className="flex-[2] bg-orange-800 text-white py-4 rounded-xl font-black uppercase text-[10px] flex items-center justify-center gap-2 shadow-md hover:bg-orange-900 transition-colors">
    ...
    return (
      <div className="bg-stone-200 min-h-screen pb-20 relative font-sans">
                  ...
                  {customItems.map((item, idx) => (
                    <tr key={idx} className="border-b border-stone-50 bg-stone-50/50">
                      <td className="py-3 flex items-center gap-2 uppercase italic text-stone-500 font-medium">
                         <button onClick={() => removeCustomItem(idx)} className="text-stone-300 hover:text-red-500 print:hidden transition-colors"><Trash2 size={10}/></button>
                         {item.name}
                      </td>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t flex gap-2 max-w-md mx-auto print:hidden shadow-lg z-[100]">
           ...
           <button onClick={async () => { await handleSaveRental(true); window.print(); }} className="flex-[2] bg-emerald-900 text-white py-4 rounded-xl font-black uppercase text-[10px] flex items-center justify-center gap-2 shadow-md hover:bg-emerald-800 transition-colors">
