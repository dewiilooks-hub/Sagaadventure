...
const TicketPage: React.FC<TicketPageProps> = ({ onBack, trips = [], onSave }) => {
  ...
  const downloadTicketPDF = async () => {
    ...
    const applyExportStyles = (root: HTMLElement) => {
      ...
      // Tailwind's `truncate` utility sets `overflow: hidden` (which can clip
      // tall glyph ascenders in html2canvas renders). For export, we relax it.
      ...
      if (holderName) {
        ...
        holderName.style.overflow = 'visible';
        holderName.style.textOverflow = 'clip';
        holderName.style.lineHeight = '1.25';
        // Move text down slightly during export to avoid top cropping.
        holderName.style.transform = 'translateY(6px)';
        holderName.style.paddingTop = '0px';
