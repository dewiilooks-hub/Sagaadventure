...
const TicketPage: React.FC<TicketPageProps> = ({ onBack, trips = [], onSave }) => {
  ...
  const downloadTicketPDF = async () => {
    ...
    // --- Export-time style patching (prevents html2canvas text clipping issues) ---
    // We patch via inline styles (instead of relying only on CSS) because html2canvas
    // can render web fonts with slightly different ascent/leading and clip the top of
    // bold uppercase glyphs when line-height is tight.
    const restoreOps: Array<() => void> = [];
    const applyTempStyle = (el: HTMLElement | null, styles: Partial<CSSStyleDeclaration>) => {
      if (!el) return;
      const prev: Partial<Record<keyof CSSStyleDeclaration, string>> = {};
      for (const [k, v] of Object.entries(styles)) {
        const key = k as keyof CSSStyleDeclaration;
        // @ts-ignore
        prev[key] = el.style[key] as string;
        // @ts-ignore
        el.style[key] = v as any;
      }
      restoreOps.push(() => {
        for (const [k, _v] of Object.entries(styles)) {
          const key = k as keyof CSSStyleDeclaration;
          // @ts-ignore
          el.style[key] = (prev[key] ?? '') as any;
        }
      });
    };
    try {
      ...
      // Apply export-only tweaks (inline styles) to prevent clipping
      const root = ticketRef.current;
      root.setAttribute('data-exporting', 'true');

      // 1) Allow content to overflow during capture (many containers use overflow-hidden)
      applyTempStyle(root, { overflow: 'visible' });
      root.querySelectorAll<HTMLElement>('.overflow-hidden').forEach(node => {
        applyTempStyle(node, { overflow: 'visible' });
      });

      // 2) Fix the ticket holder name specifically (top glyph clipping)
      const holderName = root.querySelector<HTMLElement>('.ticket-holder-name');
      applyTempStyle(holderName, {
        lineHeight: '1.15',
        paddingTop: '3px',
        paddingBottom: '1px'
      });
      ...
      // restore any temporary inline styles
      for (let i = restoreOps.length - 1; i >= 0; i--) restoreOps[i]();
