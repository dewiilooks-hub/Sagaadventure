...
export default function App() {
  ...
  // Module-scope login:
  // After logging into one module (Trip/Rental/Merch), the same session should NOT be able
  // to access the other modules without re-login.
  // We enforce this by locking the current auth session to a single module.
  const [moduleScope, setModuleScope] = useState<ViewMode | null>(() => {
    const v = sessionStorage.getItem('saga_module_scope');
    return (v as ViewMode) || null;
  });
  ...
  useEffect(() => {
    ...
    const unsub = auth.onAuthStateChanged((u) => {
      setUser(u);
      if (!u) {
        // when signed out, clear module scope
        setModuleScope(null);
        sessionStorage.removeItem('saga_module_scope');
      }
    });
    return () => unsub();
  }, []);
  ...
  const handleNavigate = (view: ViewMode) => {
    ...
    // If already logged in but session is locked to another module,
    // force re-login for the requested module.
    if (isCloudEnabled && user && moduleScope && moduleScope !== view) {
      // End current session then prompt login for the new module.
      auth?.signOut();
      setUser(null);
      setModuleScope(null);
      sessionStorage.removeItem('saga_module_scope');
    ...
    // Lock scope on first module entry when logged in.
    if (isCloudEnabled && user && !moduleScope) {
      setModuleScope(view);
      sessionStorage.setItem('saga_module_scope', view);
    }
      ...
      return (
        <LoginPage
          ...
          onLoginSuccess={() => {
            ...
            if (pendingView) {
              setModuleScope(pendingView);
              sessionStorage.setItem('saga_module_scope', pendingView);
            }
            setShowLogin(false);
